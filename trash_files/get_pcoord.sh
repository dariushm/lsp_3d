...
DIST=_lsp1_dist_$$.xvg
DIST_OUT=_lsp1_dist_$$.out.xvg
RMSD=_lsp1_rmsd_$$.xvg
RMSD_OUT=_lsp1_rmsd_$$.out.xvg
...
if [[ ${G_DIST} ]]; then
    ...
    $GROMPP -f $MDP_LOC -c $WEST_STRUCT_DATA_REF.gro -p $TOP_LOC \
          -t $WEST_STRUCT_DATA_REF.trr -o $WEST_STRUCT_DATA_REF.tpr -po md_out.mdp -n $NDX_LOC -maxwarn 2
    rm md_out.mdp

    # Update the command, then calculate the first dimension of the progress coordinate: end to end distance.
    COMMAND="18 \n 19 \n"
    echo -e $COMMAND \
      | $G_DIST -f $WEST_STRUCT_DATA_REF.gro -s $WEST_STRUCT_DATA_REF.tpr -oh $DIST -xvg none -n $NDX_LOC || exit 1
